			...
			<? if (strpos(page()->path, $page->path) === 0): ?>
				<? if ($child->getChildren()): ?>
					<li class="hover:bg-gray-100 items-center transition-colors duration-500 ease-in-out rounded-t p-2 flex justify-between relative<?= strpos(page()->path, $child->path) === 0 ? ' bg-gray-100 is-active' : '' ?>">
						...
						<svg id="openMenu" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-8 h-8 text-gray-600" :aria-expanded="isOpen" aria-controls="nav-list" :hidden="isOpen" @click="isOpen = !isOpen">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
						</svg>
						<svg id="closedMenu" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-8 h-8 text-gray-600" :aria-expanded="isOpen" aria-controls="nav-list" :hidden="!isOpen" @click="isOpen = !isOpen">
